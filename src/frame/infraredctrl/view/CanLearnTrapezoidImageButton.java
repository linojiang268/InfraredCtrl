...
/**
 * 
 * @ClassName CanLearnTrapezoidImageButton
 * @Description 可学习的不规则图标按钮
 * @author ouArea
 * @date 2013-11-25 下午4:49:08
 * 
 */
public class CanLearnTrapezoidImageButton extends TrapezoidImageButton implements CanLearnInter {
	...
	public CanLearnTrapezoidImageButton(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
	}

	@SuppressWarnings("deprecation")
	public CanLearnTrapezoidImageButton(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	public CanLearnTrapezoidImageButton(Context context) {
		super(context);
	}
			...
			public void run() {
				if (mLearnDbServer.hasLearn(mDeviceId, isCustomable, mTagId)) {
					...
					if (null!=mCanLearnCallBack) {
						mCanLearnCallBack.learnCheckBack(CanLearnTrapezoidImageButton.this, true, false, null);						
					}
				} else {
					isLearn = false;
					if (null!=mCanLearnCallBack) {
						mCanLearnCallBack.learnCheckBack(CanLearnTrapezoidImageButton.this, false, false, null);						
					}
				}
			}
