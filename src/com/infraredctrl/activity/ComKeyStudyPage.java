...
/**
 * 
 * @ClassName ComKeyStudyPage
 * @Description 组合键学习界面
 * @author ouArea
 * @date 2013-11-27 下午11:55:57
 * 
 */
public class ComKeyStudyPage extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (null != intent && intent.hasExtra("DeviceInfo") && intent.hasExtra("TagId")) {
		...
		Toast.makeText(ComKeyStudyPage.this, R.string.custom_key_page_error, Toast.LENGTH_SHORT).show();
		finish();
	}
	...
	private void findViews() {
		...
		mStudyProgressDialog = new MyProgressDialog(ComKeyStudyPage.this);
		...
		for (int i = 0; i < mCustomKeyTimeAdapters.length; i++) {
			mCustomKeyTimeAdapters[i] = new CustomKeyTimeAdapter(ComKeyStudyPage.this, android.R.layout.simple_spinner_item, m);
		...
		// -----------
		mCustomCommandService = new CustomCommandService(ComKeyStudyPage.this);
	...
	protected void onResume() {
		super.onResume();
		StatService.onResume(ComKeyStudyPage.this);
		MyPool.execute(new Runnable() {
			@Override
			public void run() {
				// 第一步注册网络消息回调监听器
				MyCon.registCallBack(ComKeyStudyPage.this, networkCallBack);
				// 第二步刷新对应mac的状态情况
	...
	protected void onPause() {
		StatService.onPause(ComKeyStudyPage.this);
		MyPool.execute(new Runnable() {
			@Override
			public void run() {
				MyCon.unregistCallBack(ComKeyStudyPage.this, networkCallBack);
			}
		}, MyPool.POOL_UI_CALLBACK);
		super.onPause();
	}
		...
		public void onClick(View v) {
			...
			case R.id.tvNum2:
				if (null == mCustomCommandInfos[0]) {
					Toast.makeText(ComKeyStudyPage.this, R.string.custom_key_order_study, Toast.LENGTH_SHORT).show();
					return;
				}
				mCurrentIndex = 1;
				MyCon.learn(mDeviceInfo.mac);
	...
	private void save() {
		if (metName.getText().toString() != null && metName.getText().toString().length() < 5 && metName.getText().toString().length() > 0) {
			...
			if (count > 0) {
				...
				mCustomCommandService.update(mDeviceInfo.id, mTagId, customCommandInfos);
				Toast.makeText(ComKeyStudyPage.this, R.string.cutome_key_save_success, Toast.LENGTH_SHORT).show();
				setResult(RESULT_OK);
				finish();
			} else {
				Toast.makeText(ComKeyStudyPage.this, R.string.custom_key_del_key, Toast.LENGTH_SHORT).show();
			}
		} else {
			AlertDialog.Builder builder = new AlertDialog.Builder(ComKeyStudyPage.this);
	...
	private NetworkCallBack networkCallBack = new NetworkCallBack() {
		...
		protected void lanMsg(byte cmd, String mac, String mark, String content) {
			...
			// 收到学习返回的编码
			if (CmdUtil.LEARN_BACK_SUCCESS == cmd && null != mCurrentIndex) {
				// mCommand = content;
				Toast.makeText(ComKeyStudyPage.this, R.string.custom_key_study_success, Toast.LENGTH_SHORT).show();
				// 自定义+多键
