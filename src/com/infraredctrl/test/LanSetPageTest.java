...
/**
 * 
 * @ClassName LanSetPageTest
 * @Description 局域网通信示例代码
 * @author ouArea
 * @date 2013-11-13 下午7:19:46
 * 
 */
public class LanSetPageTest extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.test_page_lanset);
		this.findViews();
	}
	...
	protected void onResume() {
		...
		this.testValueRead();
		// 第一步注册网络消息回调监听器
		MyCon.registCallBack(LanSetPageTest.this, networkCallBack);
		// 第二步刷新对应mac的状态情况
		if (null != mcurrentMac) {
	...
	protected void onPause() {
		MyCon.unregistCallBack(LanSetPageTest.this, networkCallBack);
		this.testValueSave();
		super.onPause();
	}

	private void findViews() {
		this.mProgressDialog = new ProgressDialog(LanSetPageTest.this);
	...
	private OnClickListener callClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.btCall:
				if (MyCon.search()) {
					Toast.makeText(LanSetPageTest.this, "call", Toast.LENGTH_SHORT).show();
				} else {
					Toast.makeText(LanSetPageTest.this, "no call", Toast.LENGTH_SHORT).show();
				}
				break;
			case R.id.btCallOne:
				if (!TextUtils.isEmpty(mcurrentMac)) {
					MyCon.refreshMac(mcurrentMac);
					Toast.makeText(LanSetPageTest.this, "call one", Toast.LENGTH_SHORT).show();
					updateMacStatusView();
				} else {
					Toast.makeText(LanSetPageTest.this, "no call one", Toast.LENGTH_SHORT).show();
				}
				break;
			default:
				break;
	...
	private OnClickListener clickListener = new OnClickListener() {
		@Override
		public void onClick(View v) {
			if (ConStatus.MAC_STATUS_LAN_ONLINE != MyCon.currentMacStatus(mcurrentMac)) {
				Toast.makeText(LanSetPageTest.this, "not lan online", Toast.LENGTH_SHORT).show();
				return;
			}
			switch (v.getId()) {
			case R.id.btLearn:
				if (MyCon.learn(mcurrentMac)) {
					...
					Toast.makeText(LanSetPageTest.this, "no learn", Toast.LENGTH_SHORT).show();
				}
				break;
			case R.id.btControl:
				if (null == mcurrentCommand) {
					Toast.makeText(LanSetPageTest.this, "no command", Toast.LENGTH_SHORT).show();
				} else {
					if (MyCon.control(mcurrentMac, mcurrentCommand)) {
					} else {
						Toast.makeText(LanSetPageTest.this, "no control", Toast.LENGTH_SHORT).show();
					}
				}
				break;
			default:
	...
	private NetworkCallBack networkCallBack = new NetworkCallBack() {
		...
		protected void lanMsg(byte cmd, String mac, String mark, String content) {
			...
			if (CmdUtil.LEARN_BACK_SUCCESS == cmd) {
				...
				Toast.makeText(LanSetPageTest.this, "learn success", Toast.LENGTH_SHORT).show();
				mProgressDialog.dismiss();
				updateMacStatusView();
			}
			// 收到控制返回
			if (CmdUtil.CONTROL_BACK_SUCCESS == cmd) {
				Toast.makeText(LanSetPageTest.this, "control success", Toast.LENGTH_SHORT).show();
			}
		}
