...
public class AddCameraActivity extends Activity implements OnClickListener, AddCameraInterface, OnItemSelectedListener, IpcamClientInterface {
		...
		public void handleMessage(Message msg) {
			if (msg.what == 1) {
				...
				if (listAdapter.getCount() > 0) {
					...
					Toast.makeText(AddCameraActivity.this, getResources().getString(R.string.add_search_no), Toast.LENGTH_LONG).show();
					isSearched = false;//
				}
			}
		}
	...
	public void onClick(View v) {
		...
		case R.id.setting:
			if (tag == 1) {
				Intent intent1 = new Intent(AddCameraActivity.this, SettingWifiActivity.class);
				...
				Toast.makeText(AddCameraActivity.this, getResources().getString(R.string.main_setting_prompt), 0).show();
			}
			break;
		case R.id.done:
			if (tag == 1) {
				Toast.makeText(AddCameraActivity.this, "没有做任何处理1", 0).show();
			} else if (tag == 2) {
				Toast.makeText(AddCameraActivity.this, "没有做任何处理2", 0).show();
			} else {
				done();
			}
			break;
	...
	private void done() {
		...
		if (strDID.length() == 0) {
			Toast.makeText(AddCameraActivity.this, getResources().getString(R.string.input_camera_id), 0).show();
			return;
		}

		if (strUser.length() == 0) {
			Toast.makeText(AddCameraActivity.this, getResources().getString(R.string.input_camera_user), 0).show();
			return;
		}
